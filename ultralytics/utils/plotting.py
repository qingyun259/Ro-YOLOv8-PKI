...
def calculate_polygon_area(box):
...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False, area=False, origin_area=0.0):
        """Add one xyxy box to image with label."""
        if isinstance(box, torch.Tensor):
            box = box.tolist()
        if self.pil or not is_ascii(label):
            if rotated:
                if area and origin_area != 0:
                    # 计算面积
                    area = calculate_polygon_area(box) / origin_area
                    if not label:
                        label = f" Area: {area:.4f}"
                    else:
                        label += f" Area: {area:.4f}"  # 在标签中添加面积信息
                p1 = box[0]
                ...
                if area and origin_area != 0:
                    # 计算面积
                    area = calculate_rect_area(box) / origin_area
                    if not label:
                        label = f" Area: {area:.4f}"
                    else:
                        label += f" Area: {area:.4f}"  # 在标签中添加面积信息
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("runs/detect/exp")):
    ...
    """
    for m in ["Detect", "Pose", "Segment", "OBB"]:
        if m in module_type:
            return

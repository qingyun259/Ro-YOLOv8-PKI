...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, DetectAux)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect,  DetectAux, Segment, Pose,
                 OBB, OBB_Aux):
            m.inplace = inplace
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2f_ConvNext, C2f_Faster,
                     C2f_ConvNext_v1,
                     C2f_ConvNext_v2,
                     C2f_ConvNext_v3,
                     C2f_iRMB,
        ...
        elif m in (Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, DetectAux, OBB_Aux):
            ...
            c2 = ch[f[-1]]
        elif m in (BiLevelRoutingAttention, CBAM, CrissCrossAttention, Faster_CBAM, LSKBlock, MLCA):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, *args[1:]]
...
def yaml_model_load(path):
    ...
    unified_path = re.sub(r"(\d+)([nslmx])(.+)?$", r"\1\3", str(path))  # i.e. Pyramid-v8.yaml -> yolov8.yaml
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, DetectAux)):
                return "detect"
